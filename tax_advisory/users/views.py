...
# Utilities
from .utils import generate_pdf, get_llm_response
...
def chatbot(request):
    ...
    try:
        ...
        handlers = {
            ...
            "house_property": handle_house_property,  # New state for house property income
            "capital_gain": handle_capital_gain,  # New state for capital gain questions
...
def handle_greeting(session, user_message):
    """Handle initial greeting using LLM while preventing repeated greetings."""
    # If already greeted, move directly to the "full_name" state.
    if session.get("greeted", False):
        session["chat_state"] = "full_name"  # Ensure state is updated.
        return "Hello again! What is your full name?"
    
    GREETING_PROMPT = """
    You are a friendly tax assistant. Read the user's message and respond based on these rules:
    
    IF the message is EXACTLY "hi", "hello", "hey", or "hola":
    -> Respond with exactly "GREETING:Hello! What is your full name?"
    
    IF the message looks like a name or anything else:
    -> Respond with exactly "INVALID:Please start with a greeting like 'hi' or 'hello'."
    
    Current user message: {user_message}
    
    Remember: For greetings, always prefix with "GREETING:"
    For invalid inputs, always prefix with "INVALID:"
    """
    
    try:
        user_message_clean = user_message.strip().lower()
        print(f"DEBUG: Processing greeting: '{user_message_clean}'")
        
        # Check if the user message is one of the allowed greetings.
        if user_message_clean in ['hi', 'hello', 'hey', 'hola']:
            # Mark as greeted and update the chat state.
            session["greeted"] = True  
            session["chat_state"] = "full_name"
            return "Hello! What is your full name?"
        
        # Otherwise, use LLM (if desired) or simply return an error.
        response = get_llm_response(user_message, GREETING_PROMPT)
        print(f"DEBUG: LLM Response: {response}")
        
        if response.startswith("GREETING:"):
            session["greeted"] = True
            session["chat_state"] = "full_name"
            return response.split("GREETING:", 1)[1].strip()
        elif response.startswith("INVALID:"):
            return response.split("INVALID:", 1)[1].strip()
        
        return "Please start with a greeting like 'hi' or 'hello'."
...
def handle_full_name(session, user_message):
    """Handle name extraction using LLM"""
    
    NAME_PROMPT = """
    You are a tax assistant. Extract names from user input using these rules:

    1. If input contains phrase like "my name is" or "I am", extract only the name part
    2. If input is just a name, use that directly
    3. Convert extracted name to proper case (capitalize first letter of each word)
    4. Must be at least first and last name

    Examples:
    "my name is John Smith" -> "NAME:John Smith"
    "My name is Mary Jane Wilson" -> "NAME:Mary Jane Wilson"
    "i am Alex Brown" -> "NAME:Alex Brown"
    "nilesh suryakant sanyasi" -> "NAME:Nilesh Suryakant Sanyasi"
    "john" -> "INVALID:Please provide your complete full name"
    ...
    try:
        ...
        if len(words) >= 2 and all(word.isalpha() for word in words):
            ...
            fsm = ChatFSM(session)
            fsm.set_state("pan")
            return f"Thank you, {name}! What is your PAN number?"
        
        # If simple validation fails, use LLM
        response = get_llm_response(user_message, NAME_PROMPT)
        print(f"DEBUG: LLM Response: {response}")
        
        if response.startswith("NAME:"):
            name = response.split("NAME:", 1)[1].strip()
            session["temp_data"]["full_name"] = name
            fsm = ChatFSM(session)
            fsm.set_state("pan")
            return f"Thank you, {name}! What is your PAN number?"
        ...
        return "Please provide your complete full name"
...
def handle_pan(session, user_message):
    if len(user_message) == 10:
        ...
        fsm = ChatFSM(session)
        fsm.set_state("uid")
        print(f"DEBUG: State updated to: {fsm.get_state()} and PAN saved: {session['temp_data']['pan']}")
...
def handle_uid(session, user_message):
    if len(user_message) == 12 and user_message.isdigit():
        session["temp_data"]["uid"] = user_message
        fsm = ChatFSM(session)
        fsm.set_state("dob")
        print(f"DEBUG: State updated to: {fsm.get_state()} and UID saved: {session['temp_data']['uid']}")
...
def handle_email(session, user_message):
    if re.match(r"[^@]+@[^@]+\.[^@]+", user_message):
        ...
        print(f"DEBUG: State updated to: {fsm.get_state()} and Email saved: {session['temp_data']['email']}")
...
def handle_confirm(session, user_message):
    if user_message.lower() == "yes":
        ...
        return "Would you like to provide your salary income details? (yes/no)"
    else:
        session.flush()
...
INCOME_QUESTIONS = [
    ...
    {"key": "tan", "question": "Please enter your TAN:"},
    {"key": "salary_income", "question": "Please enter your Salary Income:"},
    {"key": "apply_house_rent", "question": "Do you want to apply for House Rent Exemption? (yes/no)"},
    {"key": "house_rent", "question": "Please enter your Monthly House Rent:"},
    {"key": "landlord_pan", "question": "Please enter your Landlord's PAN:"},
    {"key": "apply_travel_allowance", "question": "Do you have Travel Allowance? (yes/no)"},
    {"key": "travel_allowance", "question": "Please enter your Travel Allowance amount:"},
...
def process_income_confirmation(session, user_message):
    ...
    try:
        ...
        if user_message.lower() == "yes":
            income_data = session.get("income_data", {})
            # Validate numeric fields one more time
            try:
                ...
                return "Invalid numeric values in income data. Please start over."
            ...
            session.modified = True
            fsm = ChatFSM(session)
            # Instead of setting state to generate_report, ask about deductions
            fsm.set_state("ask_deductions")
            return ("✅ Income details confirmed!\n\n"
                    "Would you like to provide deduction details to help reduce your tax payable? (yes/no)")
        else:
            session.pop("income_step", None)
            session.pop("income_data", None)
            session.modified = True
            return ("🔄 Let's re-enter your income details.\n\n"
                    f"{INCOME_QUESTIONS[0]['question']}")
    except Exception as e:
        print(f"ERROR in process_income_confirmation: {str(e)}")
        session.pop("income_step", None)
        session.pop("income_data", None)
        session.modified = True
        return "An error occurred. Let's start the income details again."
...
def process_deductions_confirmation(session, user_message):
    ...
    If confirmed, proceed to house property income.
    """
    try:
        if user_message.lower() not in ["yes", "no"]:
            return "Please answer with 'yes' or 'no'."
        if user_message.lower() == "yes":
            ...
            session.modified = True
            # Set state to house property income next
            fsm = ChatFSM(session)
            fsm.set_state("house_property")
            return "🏠 Let's now discuss your house property income.\n" + HOUSE_PROPERTY_QUESTIONS[0]["question"]
        else:
            session.pop("deduc_step", None)
            session.pop("deductions_data", None)
            session.modified = True
            # Restart deductions section
            return ("🔄 Let's re-enter your deduction details.\n" +
        ...
        session.pop("deductions_data", None)
        session.modified = True
        return "An error occurred. Please re-enter your deduction details."



# Updated HOUSE_PROPERTY_QUESTIONS with new question for House Rent Income
HOUSE_PROPERTY_QUESTIONS = [
    {"key": "num_properties", "question": "How many properties do you own?"},
    {"key": "has_rental_income", "question": "Are there properties on rent? (yes/no)"},
    {"key": "rental_income", "question": "What is the total rental income?"},
    {"key": "municipal_tax", "question": "What are the Municipal Tax charges?"},
    {"key": "maintenance_charges", "question": "What are the Maintenance Charges?"},
    {"key": "house_rent_income", "question": "What is your House Rent Income?"}  # New question added
]

def handle_house_property(session, user_message):
    """
    Handle the house property income questionnaire.
    """
    if "house_property_step" not in session:
        session["house_property_step"] = 0
        session["house_property_data"] = {}

    step = session["house_property_step"]
    house_property_data = session.get("house_property_data", {})

    if step >= len(HOUSE_PROPERTY_QUESTIONS):
        return "❌ Unexpected error: Step out of range. Please restart the house property process."

    # Get the current question
    current_question = HOUSE_PROPERTY_QUESTIONS[step]
    current_key = current_question["key"]
    answer = user_message.strip()

    # Process the answer
    if current_key == "num_properties":
        try:
            num_properties = int(answer)
            if num_properties < 0:
                return "❌ Number of properties must be a non-negative integer."
            house_property_data[current_key] = num_properties
        except ValueError:
            return "❌ Invalid input! Please enter a valid number of properties."
    elif current_key == "has_rental_income":
        if answer.lower() not in ["yes", "no"]:
            return "❌ Invalid input! Please answer with 'yes' or 'no'."
        house_property_data[current_key] = answer.lower()
        # If no rental income, skip rental income questions including House Rent Income
        if answer.lower() == "no":
            step = len(HOUSE_PROPERTY_QUESTIONS)  # Skip to end
    elif current_key in ["rental_income", "municipal_tax", "maintenance_charges", "house_rent_income"]:
        try:
            amount = float(answer.replace(',', ''))
            if amount < 0:
                return f"❌ {current_question['question']} must be a non-negative number."
            house_property_data[current_key] = amount
        except ValueError:
            return f"❌ Invalid input! Please enter a numeric value for {current_question['question']}."
    else:
        house_property_data[current_key] = answer

    # Determine next step
    step += 1
    session["house_property_step"] = step
    session["house_property_data"] = house_property_data
    session.modified = True

    # Once all questions are answered, perform calculations
    if step >= len(HOUSE_PROPERTY_QUESTIONS):
        rental_income = house_property_data.get("rental_income", 0)
        municipal_tax = house_property_data.get("municipal_tax", 0)
        maintenance_charges = house_property_data.get("maintenance_charges", 0)
        house_rent_income = house_property_data.get("house_rent_income", 0)  # New field
        
        net_rental_income = rental_income - (municipal_tax + maintenance_charges)
        deduction_e = (net_rental_income * 3) / 10

        # Save calculated values
        house_property_data["net_rental_income"] = net_rental_income
        house_property_data["deduction_e"] = deduction_e
        session["house_property_data"] = house_property_data

        # Update temp_data by including the new house rent income.
        temp_data = session.get("temp_data", {})
        temp_data["net_rental_income"] = net_rental_income
        temp_data["deduction_e"] = deduction_e
        # Include house rent income in secondary income calculation
        temp_data["secondary_income"] = temp_data.get("primary_income", 0) + net_rental_income + house_rent_income
        temp_data["house_rent_income"] = house_rent_income
        session["temp_data"] = temp_data

        fsm = ChatFSM(session)
        # NEW: Set state to 'capital_gain' instead of 'generate_report'
        fsm.set_state("capital_gain")
        return (
            f"✅ House Property details saved!\n\n"
            f"🏠 Net Rental Income: ₹{net_rental_income:,.2f}\n"
            f"📉 Deduction (30% of Rental Income): ₹{deduction_e:,.2f}\n"
            f"💼 House Rent Income: ₹{house_rent_income:,.2f}\n"
            f"💼 Secondary Income: ₹{temp_data['secondary_income']:,.2f}\n\n"
            "Now, let's discuss your Capital Gain details."
        )

    # Otherwise, ask the next question
    return HOUSE_PROPERTY_QUESTIONS[step]["question"]


# Define the capital gain questions
CAPITAL_GAIN_QUESTIONS = [
    {"key": "sold_capital_assets", "question": "Did you sell any capital assets last year? (yes/no)"},
    {"key": "asset_type", "question": "What type of asset did you sell? (Real Estate / Stocks and Mutual Funds / Other Assets)"},
    # --- Branch for Real Estate ---
    {"key": "realestate_sold_before", "question": "For Real Estate, was the property sold before 23rd July 2024? (yes/no)"},
    {"key": "realestate_purchase_date", "question": "When was the Real Estate asset purchased? (YYYY-MM-DD)"},
    {"key": "realestate_sale_date", "question": "When was the asset sold? (YYYY-MM-DD)"},
    {"key": "realestate_details", "question": "Provide purchase deed, sale deed, and expense or indexed cost details as applicable."},
    {"key": "profit_A", "question": "What is the profit made from the Real Estate sale?"},
    # --- Branch for Stocks and Mutual Funds ---
    {"key": "stocks_listed", "question": "For Stocks/Mutual Funds, was the asset listed on a stock exchange? (yes/no)"},
    {"key": "stt_paid", "question": "Was STT paid during purchase or sale? (yes/no)"},
    {"key": "profit_B", "question": "What is the profit made from Stocks/Mutual Funds?"},
    # --- Branch for Other Assets ---
    {"key": "other_stt_paid", "question": "For Other Assets, was STT paid during purchase or sale? (yes/no)"},
    {"key": "other_held_over_3_years", "question": "For Other Assets, was the asset held for more than 3 years? (yes/no)"},
    {"key": "other_indexing_applicable", "question": "If held over 3 years, is indexing applicable? (yes/no)"},
    {"key": "profit_C", "question": "What is the profit made from Other Assets?"},
    # Supporting document upload may be handled separately.
]

def handle_capital_gain(session, user_message):
    """
    Process the Capital Gain and Other Income questionnaire.
    This function is similar in structure to the income or deductions handlers and
    stores values for Profit_A, Profit_B, and Profit_C.
    """
    if "capgain_step" not in session:
        session["capgain_step"] = 0
        session["capgain_data"] = {}

    step = session["capgain_step"]
    capgain_data = session.get("capgain_data", {})

    if step >= len(CAPITAL_GAIN_QUESTIONS):
        return "❌ Unexpected error: Step out of range. Please restart the capital gains process."

    current_question = CAPITAL_GAIN_QUESTIONS[step]
    current_key = current_question["key"]
    answer = user_message.strip()

    # Process answer for yes/no or numeric/text based on current_key.
    if current_key in ["sold_capital_assets", "asset_type", "realestate_sold_before", 
                       "stocks_listed", "stt_paid", "other_stt_paid", 
                       "other_held_over_3_years", "other_indexing_applicable"]:
        capgain_data[current_key] = answer.lower()
    elif current_key in ["realestate_purchase_date", "realestate_sale_date"]:
        # (You may wish to validate the date format here.)
        capgain_data[current_key] = answer
    elif current_key in ["realestate_details"]:
        capgain_data[current_key] = answer  # Text details
    elif current_key in ["profit_A", "profit_B", "profit_C"]:
        try:
            profit = float(answer.replace(',', ''))
            if profit < 0:
                return f"❌ {current_question['question']} must be a non-negative number."
            capgain_data[current_key] = profit
        except ValueError:
            return f"❌ Invalid input! Please enter a numeric value for {current_question['question']}."
    else:
        capgain_data[current_key] = answer
    ...
    step += 1
    session["capgain_step"] = step
    session["capgain_data"] = capgain_data
    session.modified = True

    # When the questionnaire is finished
    if step >= len(CAPITAL_GAIN_QUESTIONS):
        # Calculate combined profit
        profit_A = capgain_data.get("profit_A", 0)
        profit_B = capgain_data.get("profit_B", 0)
        profit_C = capgain_data.get("profit_C", 0)
        capital_gain = profit_A + profit_B + profit_C
        # Save results into temp_data
        temp_data = session.get("temp_data", {})
        temp_data["profit_A"] = profit_A
        temp_data["profit_B"] = profit_B
        temp_data["profit_C"] = profit_C
        temp_data["capital_gain"] = capital_gain
        # Compute Tertiary Income: Secondary Income + Capital Gain (Other Income)
        secondary_income = temp_data.get("secondary_income", 0)
        temp_data["tertiary_income"] = secondary_income + capital_gain
        session["temp_data"] = temp_data

        # Set state to 'advance_tax' instead of directly generating the report.
        fsm = ChatFSM(session)
        fsm.set_state("advance_tax")


        return (f"✅ Capital Gain details saved!\n\n"
                f"💹 Combined Capital Gain: ₹{capital_gain:,.2f}\n"
                f"💼 Tertiary Income: ₹{temp_data['tertiary_income']:,.2f}\n\n"
                "Now, let's proceed to Advance Tax/TDS details.\n"
                + ADVANCE_TAX_QUESTIONS[0]["question"])
    # Otherwise, ask the next question.
    return CAPITAL_GAIN_QUESTIONS[step]["question"]



ADVANCE_TAX_QUESTIONS = [
    {"key": "tds_salary", "question": "Has TDS been deducted from your salary? (yes/no)"},
    {"key": "form16_details", "question": "If yes, please provide Form 16 details and password of your Income Tax ID:"},
    {"key": "additional_tds", "question": "Any additional TDS deducted on FDs, Interest or Rent Payments? (yes/no)"},
    {"key": "additional_tds_details", "question": "If yes, please provide details of additional TDS:"},
    {"key": "advance_tax_paid", "question": "Have you paid any Advance Tax or Self-Assessment Tax? (yes/no)"},
    {"key": "form26as_details", "question": "If yes, please provide Form 26AS details and password of your Income Tax ID:"},
]

def handle_advance_tax(session, user_message):
    if "advance_tax_step" not in session:
        session["advance_tax_step"] = 0
        session["advance_tax_data"] = {}
    step = session["advance_tax_step"]
    advance_tax_data = session.get("advance_tax_data", {})
    
    if step >= len(ADVANCE_TAX_QUESTIONS):
        return "❌ Unexpected error in Advance Tax section. Please restart."
    
    current_question = ADVANCE_TAX_QUESTIONS[step]
    current_key = current_question["key"]
    answer = user_message.strip()
    
    # For yes/no questions:
    if current_key in ["tds_salary", "additional_tds", "advance_tax_paid"]:
        if answer.lower() not in ["yes", "no"]:
            return "❌ Please answer with 'yes' or 'no'."
        advance_tax_data[current_key] = answer.lower()
        # If answer is "no", skip the detail question that follows.
        if current_key == "tds_salary" and answer.lower() == "no":
            step += 2  # Skip form16_details.
        elif current_key == "additional_tds" and answer.lower() == "no":
            step += 2  # Skip additional_tds_details.
        elif current_key == "advance_tax_paid" and answer.lower() == "no":
            step += 2  # Skip form26as_details.
        else:
            step += 1
    else:
        # For detail questions: store the text.
        advance_tax_data[current_key] = answer
        step += 1
    
    session["advance_tax_step"] = step
    session["advance_tax_data"] = advance_tax_data
    session.modified = True
    
    if step >= len(ADVANCE_TAX_QUESTIONS):
        # Save Advance Tax details in temp_data then move to Section E.
        temp_data = session.get("temp_data", {})
        temp_data["advance_tax_data"] = advance_tax_data
        session["temp_data"] = temp_data
        fsm = ChatFSM(session)
        fsm.set_state("other_disclosures")
        return "✅ Advance Tax/TDS details saved!\n\nNow, let's move to Other Disclosures & Compliance.\n" + OTHER_DISCLOSURES_QUESTIONS[0]["question"]
    
    return ADVANCE_TAX_QUESTIONS[step]["question"]



OTHER_DISCLOSURES_QUESTIONS = [
    {"key": "has_foreign_assets", "question": "Do you hold any foreign bank account, foreign assets, or investments? (yes/no)"},
    {"key": "foreign_assets_details", "question": "If yes, please provide details of your foreign bank account, assets, or investments:"},
    {"key": "received_gifts", "question": "Have you received gifts above ₹50,000? (yes/no)"},
    {"key": "gift_amount", "question": "If yes, please provide the total amount of gifts received:"},
]

def handle_other_disclosures(session, user_message):
    if "other_disc_step" not in session:
        session["other_disc_step"] = 0
        session["other_disc_data"] = {}
    step = session["other_disc_step"]
    other_disc_data = session.get("other_disc_data", {})
    
    if step >= len(OTHER_DISCLOSURES_QUESTIONS):
        return "❌ Unexpected error in Other Disclosures section. Please restart."
    
    current_question = OTHER_DISCLOSURES_QUESTIONS[step]
    current_key = current_question["key"]
    answer = user_message.strip()
    
    if current_key in ["has_foreign_assets", "received_gifts"]:
        if answer.lower() not in ["yes", "no"]:
            return "❌ Please answer with 'yes' or 'no'."
        other_disc_data[current_key] = answer.lower()
        # If answer is "no", skip the next detail question.
        if current_key == "has_foreign_assets" and answer.lower() == "no":
            step += 2  # Skip foreign_assets_details.
        elif current_key == "received_gifts" and answer.lower() == "no":
            step += 2  # Skip gift_amount.
        else:
            step += 1
    elif current_key == "gift_amount":
        try:
            gift_amt = float(answer.replace(',', ''))
            if gift_amt < 0:
                return "❌ Gift amount must be a non-negative number."
            other_disc_data[current_key] = gift_amt
            # Compute taxable amount = gift_amt - 50000 (if applicable).
            other_disc_data["gift_taxable_amount"] = gift_amt - 50000 if gift_amt > 50000 else 0
        except ValueError:
            return "❌ Please enter a valid numeric gift amount."
        step += 1
    else:
        # For foreign_assets_details.
        other_disc_data[current_key] = answer
        step += 1
    
    session["other_disc_step"] = step
    session["other_disc_data"] = other_disc_data
    session.modified = True
    
    if step >= len(OTHER_DISCLOSURES_QUESTIONS):
        # Save Other Disclosures details and proceed.
        temp_data = session.get("temp_data", {})
        temp_data["other_disc_data"] = other_disc_data
        session["temp_data"] = temp_data
        fsm = ChatFSM(session)
        fsm.set_state("bank_details")
        return "✅ Other Disclosures & Compliance details saved!\n\nNow, please provide your bank account details for the refund.\n" + BANK_DETAILS_QUESTIONS[0]["question"]
    
    return OTHER_DISCLOSURES_QUESTIONS[step]["question"]



BANK_DETAILS_QUESTIONS = [
    {"key": "bank_name", "question": "Please provide your Bank Name:"},
    {"key": "account_number", "question": "Please provide your Account Number:"},
    {"key": "ifsc_code", "question": "Please provide your IFSC Code:"},
]

def handle_bank_details(session, user_message):
    if "bank_details_step" not in session:
        session["bank_details_step"] = 0
        session["bank_details_data"] = {}
    step = session["bank_details_step"]
    bank_details_data = session.get("bank_details_data", {})
    
    if step >= len(BANK_DETAILS_QUESTIONS):
        return "❌ Unexpected error in Bank Details section. Please restart."
    
    current_question = BANK_DETAILS_QUESTIONS[step]
    current_key = current_question["key"]
    answer = user_message.strip()
    
    bank_details_data[current_key] = answer  # Save as text.
    step += 1
    session["bank_details_step"] = step
    session["bank_details_data"] = bank_details_data
    session.modified = True
    
    if step >= len(BANK_DETAILS_QUESTIONS):
        # Save Bank Details and proceed to final report generation.
        temp_data = session.get("temp_data", {})
        temp_data["bank_details"] = bank_details_data
        session["temp_data"] = temp_data
        fsm = ChatFSM(session)
        fsm.set_state("generate_report")
        return "✅ Bank Account Details saved!\n\nGenerating your tax report now..."
    
    return BANK_DETAILS_QUESTIONS[step]["question"]
    ...
    try:
        with transaction.atomic():
            ...
            basic_data = TempUserData.objects.create(
                ...
                pan=temp_data.get('pan', '').strip(),
                uid=temp_data.get('uid', '').strip(),
            ...
            merged_data = {
                ...
                'house_rent_income': temp_data.get('house_rent_income', 0),  # Add House Rent Income
                # Deductions:
                'deductions_80C': {
                    'epf': epf,
                    'life_insurance': life,
                ...
                'net_rental_income': temp_data.get('net_rental_income', 0),  # Net Rental Income
                'deduction_e': temp_data.get('deduction_e', 0),  # Deduction (30% of Rental Income)
                'secondary_income': temp_data.get('secondary_income', 0),  # Secondary Income
                'capital_gain': temp_data.get('capital_gain', 0),
                'tertiary_income': temp_data.get('tertiary_income', 0),
