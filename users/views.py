...
def handle_greeting(session, user_message):
    """Handle initial greeting using LLM"""
    ...
    try:
        ...
        # Debug print
        print(f"DEBUG: Processing greeting: '{user_message}'")
        
        # Simple greeting check before calling LLM
...
def process_deductions_confirmation(session, user_message):
    ...
    try:
        ...
        if user_message.lower() == "yes":
            ...
            session.modified = True
            # Set state to generate the report next.
            fsm = ChatFSM(session)
            fsm.set_state("generate_report")
            return ("✅ Deductions confirmed!\nGenerating your tax report now...")
        else:
            session.pop("deduc_step", None)
            session.pop("deductions_data", None)
            session.modified = True
